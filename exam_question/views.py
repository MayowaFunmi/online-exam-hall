...
from exams.models import Question, Department, StudentClass, StudentLevel, Subjects, ExaminerExamRegistration


def choose_exam(request):
    if ExaminerExamRegistration.objects.filter(examiner=request.user).exists():
        return render(request, 'exam_question/choose_exam.html')
    else:
        return HttpResponseRedirect('/exam_question/exam_register/')


def examiner_subject(request):
    registered = ExaminerExamRegistration.objects.filter(examiner=request.user)
    return render(request, 'exam_question/select_subject.html', {'registered': registered})


def view_examiner_questions(request):
    registered = ExaminerExamRegistration.objects.filter(examiner=request.user)
    return render(request, 'exam_question/question/view_examiner_questions.html', {'registered': registered})


#out
def set_exam(request):
    registered = ExaminerExamRegistration.objects.filter(examiner=request.user)
    return render(request, 'exam_question/examiner_questions.html', {'registered': registered})
...
# financial accounts question for examiners
