...
#check student exam reg status
def check_register_exam(request):
    if ExamRegistration.objects.filter(candidate=request.user).exists():    # not probably needed
        return render(request, 'exams/registered.html')
    else:
        subjects = Subjects.objects.all()
        return render(request, 'exams/check_reg_status.html', {'subjects': subjects})
...
def all_departments(request):
    ...
    return render(request, 'exams/all_departments.html', {'depts': depts})
...
def edit_department(request):
    if request.user.is_staff:
        ...
        messages.error(request, 'You Are Not Authorized To Edit A Department')
        return render(request, 'exams/edit_department.html')
...
def all_subjects(request):
    subjects = Subjects.objects.all()
    return render(request, 'exams/all_subjects.html', {'subjects': subjects})
    ...
    if request.user.is_staff and request.method == 'POST':
        ...
        try:
            subject_add = Subjects(admin=admin, subject_name=subject_name, subject_code=subject_code,
                                   department=department)
            subject_add.save()
            message = subject_name + '('+subject_code+')'
