...
def update_candidate(request):
    context = {}
    obj = get_object_or_404(CandidateProfile, candidate=request.user)
    form = CandidateProfileForm(data=request.POST or None, files=request.FILES or None, instance=obj)
...
def update_examiner(request):
    context = {}
    obj = get_object_or_404(ExaminerProfile, examiner=request.user)
    form = ExaminerProfileForm(data=request.POST or None, files=request.FILES or None, instance=obj)
    if form.is_valid():
        form.save(commit=True)
        messages.success(request, 'Profile Updated Successfully')
